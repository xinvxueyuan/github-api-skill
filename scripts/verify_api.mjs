...
// - verify GitHub's current REST API version matches the version declared in docs
...
const VARS = { '{o}': 'octocat', '{r}': 'Hello-World', '{u}': 'octocat', '{org}': 'octocat', '{n}': '1', '{id}': '1', '{b}': 'main', '{name}': 'readme', '{path}': 'readme', '{ref}': 'heads/main', '{commit}': 'main', '{sha}': 'main', '{base}': 'main', '{head}': 'main', '{label}': 'readme', '{team}': '1', '{username}': 'octocat' }

const md = await readFile(REF, 'utf8')

// Version declared in the docs (rest-core.md), e.g. "X-GitHub-Api-Version: 2026-03-10".
const mdVer = md.match(/X-GitHub-Api-Version:\s*([0-9]{4}-[0-9]{2}-[0-9]{2})/)
const EXPECTED = mdVer ? mdVer[1] : '2022-11-28'
...
// Paths whose endpoints hide existence behind 401/403/404 (auth/permission dependent):
// a 404 here is NOT evidence of endpoint removal.
const AUTH_SENSITIVE = /(audit-log|secrets|collaborators|invitations|keys|installations)/
...
async function liveCheck(path) {
  ...
  const headers = { Accept: 'application/vnd.github+json', 'User-Agent': 'github-api-skill-freshness', 'X-GitHub-Api-Version': EXPECTED }
...
for (const p of unique) {
  ...
  else if (code === 404) verdict = (isScalar(p) || AUTH_SENSITIVE.test(p)) ? 'info' : 'fail-endpoint'
  else if (code === 403 || code === 429) verdict = 'warn-limit'
  else if (code === 401) verdict = 'warn-auth'
...
// Version drift: the API must accept the version declared in the docs.
const apiVersions = [...new Set(results.map((x) => x.apiVer).filter(Boolean))]
let versionDrift = null
if (apiVersions.length && apiVersions.some((v) => v !== EXPECTED)) {
  versionDrift = { path: '(API 版本)', url: 'https://api.github.com/', status: 'MISMATCH', note: 'docs 声明 ' + EXPECTED + '，实测响应头 ' + apiVersions.join(' / ') }
  fails.push(versionDrift)
}
...
lines2.push('- **实测响应头 API 版本**：' + (apiVersions.length ? apiVersions.join(' / ') : '（未返回版本头）'))
...
lines2.push('')
if (fails.length) {
  lines2.push('## 疑似漂移（端点缺失、版本不匹配或网络错误）')
  lines2.push('')
  ...
  lines2.push('## 无漂移')
  lines2.push('')
  lines2.push('所有文档化端点均可解析（GET 校验），API 版本与文档声明一致。')
  lines2.push('')
}
lines2.push('> 状态：`ok` 通过 · `info` 单资源 404 / 需认证（正常）· `warn-*` 限流/权限/服务器 · `fail-endpoint` 端点疑似被移除或改名。')
...
console.log(JSON.stringify({ checkedAt: iso, expectedVersion: EXPECTED, apiVersions, total: unique.length, ok, info, warns, failures: fails.length }, null, 2))
